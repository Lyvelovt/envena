...
from src.envena.utils.functions import validate_ip, get_vendor, get_hostname
            ...
            if source_ip in ARP_TABLE:
                if ARP_TABLE[source_ip] != source_mac:
                    ...
                    if source_mac != "ff:ff:ff:ff:ff:ff":
                        ws.set_host(mac=source_mac, ip=source_ip, vendor=get_vendor(source_mac), hostname=get_hostname(source_ip))
            else:
                ARP_TABLE[source_ip] = source_mac
