...
from scapy.all import ICMP, IP, conf, get_if_addr, get_if_hwaddr, sniff
...
def icmpmap(param, logger, ws=None):
    ...
    for ip in hops:
        ...
        for i in range(0, len(hops[ip])):
            logger.info(
                f"Hop {i + 1}...: ip: {hops[ip][i][0]}, time: {round(hops[ip][i][1], 4)} sec."
            )
            # print(hops)
            # break
...
class t_icmpmap(BaseTool):
    """
    Map network topology using ICMP Echo requests.

    Arguments:
        ip_dst (Required): Destination host for path discovery (e.g. 8.8.8.8).
        input (Required): Subnet range to scan for active nodes.
        iface (Optional): Network interface.

    Example:
        args set ip_dst 8.8.8.8
...
if __name__ == "__main__":
    try:
        ...
        parser.add_argument(
            "-id", "--ip_dst", help="destination IP-address", required=True, type=str
        )
        parser.add_argument(
            "-ip",
            "--ip",
            help="IP-address(es) range of subnetwork",
            required=True,
            type=str,
        )
        parser.add_argument(
