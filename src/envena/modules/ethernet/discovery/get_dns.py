...
def get_dns(param, logger, ws) -> None:
    logger.info(
        f'DNS response: {param.input} is "{get_hostname(ip=param.input, iface=param.iface, dns_server=str(param.dns_server))}"'
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("-ip", help="target IP address", required=True, type=str)
    parser.add_argument(
        "-i",
        "--iface",
        help="network interface to send request from",
        default=str(conf.iface),
    )
    parser.add_argument(
        "-ds",
        "--dns_server",
        help="DNS server to send request",
        required=False,
        type=str,
        default="8.8.8.8",
    )
