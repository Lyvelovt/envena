...
class Workspaces:
    ...
    def current(self, name: str):
        ...
        with self.conn:
            ...
            self.conn.execute("PRAGMA journal_mode = WAL;") # High-performance concurrent mode
            self.conn.execute("PRAGMA busy_timeout = 5000;") # Wait up to 5s if DB is locked
    ...
    def get_host_id(self, mac: str = None, ip: str = None) -> Optional[int]:
        ...
        if mac:
            res = self.conn.execute("SELECT id FROM hosts WHERE mac = ?", (mac.lower().strip(),)).fetchone()
            if res: return res[0]
        
        if ip:
            res = self.conn.execute("SELECT id FROM hosts WHERE ip = ? ORDER BY last_seen DESC", (ip,)).fetchone()
            if res: return res[0]
    ...
    def set_service(self, host_id: int, port: int, name: str = None, ver: str = None):
        ...
        with self.conn:
            ...
            res = self.conn.execute("SELECT id FROM services WHERE host_id=? AND port=?", (host_id, port)).fetchone()
            return res[0]