from scapy.all import ICMP, IP, Ether, hexdump, sendp


def send_icmp_echo_request(param, verbose=True) -> bool:
    ...
    packet = (
        ...
        / IP(dst=ip_dst, src=ip_src, ttl=ttl)
        / ICMP(id=icmp_id, seq=seq, type=8)
    )
    if payload:
        packet / payload
