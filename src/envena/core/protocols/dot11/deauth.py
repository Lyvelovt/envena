from scapy.all import Dot11, Dot11Deauth, RadioTap, hexdump, sendp


def send_deauth(param, verbose: bool = True, two_way: bool = True) -> bool:
    ...
    try:
        if not two_way:
            ...
            if verbose:
                param.logger.info(
                    f"Sent deauth frame: {hw_src} -> {hw_dst}. BSSID: {bssid}"
                )
                hexdump(packet)
                return True
        else:
            ...
            if verbose:
                param.logger.info(
                    f"Sent deauth frames: {hw_src} <-> {hw_dst}. BSSID: {bssid}"
                )
