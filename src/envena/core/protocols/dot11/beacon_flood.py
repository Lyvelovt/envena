...
from src.envena.core.config import Clear, Error_text, Fatal_Error, scapy
from src.envena.utils.functions import rand_eth, rand_ssid, validate_args


def beacon_flood(interface, count=0, timeout=0.1):
    # 802.11 frame
    dot11 = Dot11(
        type=0, subtype=8, addr1="ff:ff:ff:ff:ff:ff", addr2=rand_eth(), addr3=rand_eth()
    )
