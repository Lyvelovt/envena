...
def send_arp_response(ip_dst, eth_dst, ip_src, eth_src, iface, printed=True):
    ...
    ether = scapy.Ether(src=eth_src, dst=eth_dst)
    ...
    try:
        ...
        if printed:
            print(
                f"[{iface}] Sent ARP response: {ip_src} -> {ip_dst}: {ip_src} is at {eth_src}")
