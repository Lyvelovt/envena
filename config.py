...
# READY-MADE SCRIPTS #=======================#
from netutils.script.arpscan import *
from netutils.script.dns_getHostname import *
...
envenena_art = [
    ...
    f'{w}?          {w}55{n}5@@@%@@@@@@@5@@@@{w}5{n}5{w}5{n}@5{w}555        {Muted}~~version: 1.0~~{r}',
    ...
    f'                   {g}c{n}@@@{w}.  {g}c{w}5{n}@@@%@@@5{w}5{n}@@{w}??    {'\033[1;31m'}soon will be in open-source :){r}',
    f'                   {w}.{n}%@{w}?     ?{n}@@@5@@@@@5{w}5?',
...
Soon:
cam_overflow        -    attack on the router by overflowing the CAM table.
tcp.syn             -    send TCP packet with 'SYN' flag.
tcp.ack             -    send TCP packet with 'ACK' flag.
tcp.sa              -    send TCP packet with 'SYN-ACK' flag.
tcp.fin             -    send TCP packet with 'FIN' flag.
"""
...
def list_dict(data: Dict[str, Union[str, int]], title: str = "ARGS LIST") -> None:
    ...
    max_value_length = max(len(str(value)) for value in values) if values else 0
    max_key_length = max(max_key_length, 4)
    max_value_length = max(max_value_length, 5)

    # Divide string
    separator = "+" + "-" * (max_key_length + 2) + "+" + "-" * (max_value_length + 2) + "+"

    print(f"{title}:")

    print(separator)
    print(
        f"| Name{' ' * (max_key_length - 4)} | Value{' ' * (max_value_length - 5)} |")
    print(separator)

    for key, value in data.items():
        value_str = str(value)
        print(
            f"| {key}{' ' * (max_key_length - len(key))} | {value_str}{' ' * (max_value_length - len(value_str))} |")
    print(separator)
